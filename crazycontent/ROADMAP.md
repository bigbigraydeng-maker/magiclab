...
### 2026-05-03（五层架构 + Layer 1/2 扩展立项 + FDE 模式确立）⭐
...
### 2026-05-03（核心交付模式：FDE 一线驻场工程师）⭐⭐⭐
- **战略级决策**：确立 Magic Lab 的核心交付模式为 **Frontline Deployment Engineer（FDE，一线驻场工程师）**，对标 Palantir 的 Forward Deployed Engineer 模式
- **定位转变**：Magic Lab 对外不再定位为"内容增长服务商"或"AI 内容工具"，而是 **"由 FDE 驱动的 AI 内容增长引擎"**——我们不卖软件，我们派工程师住在客户业务里
- **关键特征**：
  - 每位客户配备 1 名（或 1 组）真正能写代码、能调系统的驻场工程师
  - FDE 60% 时间在客户业务现场（销售例会、询盘审视、内容选题），40% 时间为客户定制平台
  - 客户老板 ↔ FDE 直接沟通，0 层翻译损耗（无销售/客户经理/PM 中间层）
  - 平台为客户业务生长（每个客户的需求都沉淀回 Magic Engine 平台）
- **定价模式调整**（替换原"按陪跑等级"）：
  - **Starter** NZ$50k/年：1 名 FDE 50% 时间 + 完整平台
  - **Growth** NZ$100k/年：1 名专属 FDE + 季度战略复盘 + 平台定制权
  - **Enterprise** NZ$150k+/年：FDE 团队（2-3 人）+ 多语言 + 多市场 + 优先功能开发
  - 定价基础：**engineer proximity-based**，不按席位、不按用量
- **组织影响**（招聘 + 团队结构）：
  - 现有"全栈工程师"职位重写为 FDE JD
  - FDE 必备能力：TypeScript/Python + 业务理解 + 中英双语 + 客户会议适应能力
  - FDE 不必备能力：996、长 PRD、过度工程化
  - 薪资带：NZ$120k-180k base + 股权 + 客户成功分成
- **护城河叙事升级**：从"工具差异化"升级为 **"FDE × 数据资产复利双重壁垒"**——SaaS 公司没有 FDE 团队，传统 Agency 没有平台数据资产，复制 Magic Lab 需同时具备两者
- **对外材料**：新建 `MAGIC_LAB_INTRO.md` 作为对外正式介绍文档，FDE 作为核心叙事锚点
- **风险确认**：FDE 模式天花板大约 30-50 客户（团队带宽限制），不追求 SaaS 规模化。这个限制是**功能而非缺陷**——稀缺性即定价权
